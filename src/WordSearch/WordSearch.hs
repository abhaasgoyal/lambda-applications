...
searchingTypes grid =
  zip
    [ gridWithPos,
      ...
      diagonalString $ transpose $ reverse twoDGridWithPos
    ]
    ordOrient
  where
...
findPos (x, y) orient word = zip x_r y_r
  where
    ...
    posList =
      zip3
        [forward, back, x_s, x_s, forward, back, forward, back]
...
helpFillWord currGrid word = do
  ...
  if acceptableWord
    then return newGrid
    else helpFillWord currGrid word
  where
...
createWordSearch wordList density = do
  ...
  if correctGrid finalGrid wordList
...
finalRandFill letterRange x = do
  ...
  if x == '_'
    then return (letterRange !! randIdx)
    else return x

--- Convenience functions supplied for testing purposes
createAndSolve :: [String] -> Double -> IO [(String, Maybe Placement)]
createAndSolve wordList maxDensity = do
  g <- createWordSearch wordList maxDensity
